...
void dump_memory(runtime_t rt) {
    puts("MEMORY OCCUPATION:");
    printf(" STRINGS   = %5i / %5i (%2i%%)\n", rt->csp - rt->csp0, rt->pp0 - rt->csp0, (int)(100.0*(rt->csp - rt->csp0) / (rt->pp0 - rt->csp0)));
    printf(" PROGRAM   = %5i / %5i (%2i%%)\n", rt->pp - rt->pp0, rt->vp0 - rt->pp0, (int)(100.0*(rt->pp - rt->pp0) / (rt->vp0 - rt->pp0)));
    printf(" VARIABLES = %5i / %5i (%2i%%)\n", rt->vp - rt->vp0, rt->sp - rt->vp0, (int)(100.0*(rt->vp - rt->vp0) / (rt->sp - rt->vp0)));
    puts("MEMORY MAP:");
    puts("  | strings | program | variables | free space | stacks | buffers |");
...
void oper_empty_string(runtime_t rt)
{
...
str_t oper_concat(runtime_t rt, str_t s1, str_t s2) {
...
void OPER_EOF(runtime_t rt) {
    // Check whether a file is ended.
    int ch = pop_num(rt);
...
void OPER_HASH(runtime_t rt) {
    // Convert to integer and check against channel number
    int ch = pop_num(rt);
    if (ch < 0 || ch > BUF_NUM) ERROR(ILLEGAL_CHANNEL);
    push_num(rt, ch);
}

void OPER_IDIV(runtime_t rt) {
...
void OPER_RND(runtime_t rt) { push_num(rt, (double)rand()/RAND_MAX); }

void OPER_SGN(runtime_t rt) {
    num_t n = pop_num(rt);
...
int var_array_address(runtime_t rt, int type, addr_t p, addr_t *a1) {
    int d2 = 1, j = 1;
...
int expr_prefix_operators(runtime_t rt) {
...
void expr_operand(runtime_t rt) {
    switch (CODE) {
    case '(': {         // Subexpression.
        ++ IP;
        expr(rt);
...
Again:
    ...
    runtime_eunroll(rt, 1); // Not 0, else the NULL function is invoked!!!
...
void assign_expr(runtime_t rt, int type, addr_t v, addr_t va) {
    addr_t token_dump(runtime_t,addr_t, FILE*);
...
/// Parse a possible channel and return the index of the corresponding buffer.
int instr_channel(runtime_t rt) {
...
addr_t instr_skip_line(runtime_t rt) {
    rt->ip0 += RAM[rt->ip0];    // Skip to the next line.
    return (rt->ip0 >= rt->pp) ? IP = NIL : LINE_START;
}
...
void INSTR_DATA(runtime_t rt) { instr_skip_line(rt); }
void INSTR_DEF(runtime_t rt) { instr_skip_line(rt); }
...
void INSTR_ERROR(runtime_t rt) { IP = NIL; longjmp(rt->err_buffer, rt->error = pop_num(expr(rt))); }
...
void INSTR_RANDOMIZE(runtime_t rt) { srand(pop_num(expr(rt))); }
...
void INSTR_REM(runtime_t rt) { instr_skip_line(rt); }
...
void INSTR_RETURN(runtime_t rt) { rpop(rt, &rt->ip0, &IP); }
...
void INSTR_SKIP(runtime_t rt) { instr_skip_line(rt); instr_skip_line(rt); }
void INSTR_STEP(runtime_t rt) { ERROR(ILLEGAL_INSTRUCTION); }
void INSTR_STOP(runtime_t rt) { ERROR(STOP); }
...
int instr_exec(runtime_t rt) {
    ...
    static void (*Instructions[])(runtime_t) = {
