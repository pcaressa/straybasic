...
The screen is divided in three zones: the upper line, called *header*, contains information about the file under editing; the next lines display a portion of the file (the all file if it fits into this area). The last inserted character is marked: the cursor is just after it. The last line, the *footer*, is used to insert data, for example strings to look for. Normally, pressed keys correspond to characters to be appended to the current line in the current file.
...
- `c` copy the current line, thus store it inside a hidden variable.
- `d` duplicate the current line: same as `ESC c ESC i`.
- `f` prompt for a string and find its next occurrence: type ENTER to look for the previous searched string.
- `h` print a help message.
- `i` insert the copied line after the current one.
...
- `r` prompt for a string to look for and for a string to replace and replace the next occurrence of the search string: type ENTER to look for the previous search/replace string.
...
To check which codes are returned when escape and control characters are pressed, one may experiment with the following program:

    10 LET C = ASC(INKEY$): PRINT C: IF C <> 10 THEN 10

which keeps on printing code sequences corresponding to pressed characters until the ENTER character is pressed. On my Linux terminal I've discovered in this way that:

    up arrow returns the ASCII sequence 27 91 65
    down arrow returns the ASCII sequence 27 91 66
    right arrow returns the ASCII sequence 27 91 67
    left arrow returns the ASCII sequence 27 91 68
    Line end returns the ASCII sequence 27 91 70
    Line start returns the ASCII sequence 27 91 72
    Canc returns the ASCII sequence 27 91 51 126
    Page Up returns the ASCII sequence 27 91 53 126
    Page Down returns the ASCII sequence 27 91 54 126
    Delete returns the ASCII sequence 127
...
Since this program is long, to save space I've dropt the `LET` keywords and try to groups statements by colons whenever possible.
...
Try to use this editor to edit Basic programs: it'll be far more easy than by command line insertion. In particular, it can load itself: after that, the memory usage will be:

    MEMORY:
       STRINGS = 1075/4096 (26%); PROGRAM = 7082/8192 (86%); VARIABLES = 14569/51532 (28%)
    MEMORY MAP:
        | strings | program | variables | free space | stacks | buffers |
